...
search = {"reviewer": "","review_date": "", "review": ""}
...
def reviews(book_id):
    ...
    db.execute("INSERT INTO review (reviewer, review_date, review,books_id) VALUES (:reviewer, :review_date, :review, :books_id)",
    {"reviewer": session.get('username'), "review_date":timestamp, "author": book.author, "review":request.form['text'], "books_id": book_id})
        
    reviews = db.execute("SELECT name FROM reviews WHERE books_id = :books_id",
                            {"books_id": book_id}).fetchall()  
    review=reviews.review
    reviewer=reviews.reviewer                            
    print(review) 
    if db.execute("SELECT * FROM revews WHERE review_id = :book_id", {"book_id": book_id}).rowcount > 0:
        return redirect(url_for('book', book_book_id)) 
    
    return render_template("reviews.html", book_book_id)

@socketio.on("submit search")
def search(select,input):
    print("select = ",select)
    print("input = ",input)
    if input == "":
        book_list = db.execute("SELECT * FROM books").fetchall()
    ...
    emit("load", book_list, broadcast=True)
